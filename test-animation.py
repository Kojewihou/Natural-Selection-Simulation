...
num_organisms = 200
organisms = [
    Organism(0.5, 0.5, 5, np.random.uniform(0.01,0.03))
    for _ in range(num_organisms)
]

# Create plot elements for the organisms with different sizes
organism_plots = [
    ax.plot([], [], 'o', markersize=organism.size, color='green')[0] for organism in organisms
]
...
ani = FuncAnimation(fig, update, frames=np.arange(0, 100), blit=True, interval=interval)
